...
class Num2Word_PAP(Num2Word_Base):
    """Papiamento number-to-words converter."""

    CURRENCY_FORMS = {
        "ANG": (("florin", "florin"), ("sèn", "sèn")),
        "AWG": (("florin", "florin"), ("sèn", "sèn")),
        "USD": (("dolar", "dolar"), ("sèn", "sèn")),
        "EUR": (("euro", "euro"), ("sèn", "sèn")),
    }

    def setup(self):
        self.negword = "menos "
        self.pointword = "punto"
        self.exclude_title = ["i", "punto", "menos"]
        self.ones = [
            "", "un", "dos", "tres", "kuater", "sinku",
            "seis", "shete", "ocho", "nuebe",
        ]
        self.teens = [
            "dies", "diesun", "diesdos", "diestres", "dieskuater", "diessinku",
            "diesseis", "diesshete", "diesocho", "diesnuebe",
        ]
        self.tens = [
            "", "dies", "binti", "trinta", "kuarenta", "sinkuenta",
            "sesenta", "setenta", "ochenta", "nobenta",
        ]
        self.hundred = "shen"
        self.thousand = "mil"
        self.million = "miyon"
    ...
    def _int_to_word(self, number):
        ...
        if number < 100:
            ...
            return self.tens[t] + (" i " + self.ones[o] if o else "")
        if number < 1000:
            h, r = divmod(number, 100)
            base = (self.ones[h] + " " if h > 1 else "") + self.hundred
            return base + (" i " + self._int_to_word(r) if r else "")
        if number < 1000000:
    ...
    def to_ordinal(self, number):
        if number == 1:
            return "promé"
        return "di " + self.to_cardinal(number)
    ...
    def to_currency(self, val, currency="ANG", cents=True, separator=" ", adjective=False):
        is_negative = val < 0
        val = abs(val)
        parts = str(val).split(".")
