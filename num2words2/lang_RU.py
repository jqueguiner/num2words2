...
def case_classifier_element(
    classifier, case=D_CASE, plural=D_PLURAL, gender=D_GENDER, animate=D_ANIMATE
):
...
ONES_ORD_POSTFIXES_GROUPS = {0: 0, 1: 1, 2: 0, 4: 1, 5: 1, 6: 0, 7: 0, 8: 0, 9: 1}
CASE_POSTFIXES = [
    [{0: "ой", 1: "ый"}, "ая", "ое", "ые"],
    ["ого", "ой", "ого", "ых"],
    ["ому", "ой", "ому", "ым"],
                ...
                if isinstance(postfix, str)
                    ...
                    [
                        (
                            ...
                            else prefix + animate[post_group]
                        )
                        for animate in postfix
                    ]
                    if isinstance(postfix, list)
                    else prefix + postfix[post_group]
                )
            )
            for postfix in case
        ]
...
def get_ord_classifier(prefixes, post_groups):
    ...
    return {
        num: get_cases(prefix, post_groups[num]) for num, prefix in prefixes.items()
    }
...
ONES_ORD.update(get_ord_classifier(ONES_ORD_PREFIXES, ONES_ORD_POSTFIXES_GROUPS))
...
TENS_POSTFIXES = ["надцать", "надцати", "надцати", "надцать", "надцатью", "надцати"]
...
TWENTIES_ORD = get_ord_classifier(TWENTIES_ORD_PREFIXES, TWENTIES_ORD_POSTFIXES_GROUPS)
...
THOUSANDS.update(
    {
        num: [[prefix + postfix for postfix in case] for case in THOUSANDS_POSTFIXES]
...
class Num2Word_RU(Num2Word_Base):
    ...
    def to_cardinal(
        self, number, case=D_CASE, plural=D_PLURAL, gender=D_GENDER, animate=D_ANIMATE
    ):
        ...
        if isinstance(number, float):
            ...
            # Always preserve decimal point for floats in Russian
            n = str(number)
        else:
            n = str(number)
    ...
    def to_ordinal(
        ...
        n = str(int(number))  # Convert to int first to handle floats
        return self._int2word(
            int(n),
            cardinal=False,
            case=case,
    ...
    def _int2word(
        ...
        kwargs = {"case": case, "plural": plural, "gender": gender, "animate": animate}

        if n < 0:
            return " ".join(
                [self.negword, self._int2word(abs(n), cardinal=cardinal, **kwargs)]
            )

        if n == 0:
            return (
                get_num_element(ONES, 0, **kwargs)
                if cardinal
                else get_num_element(ONES_ORD, 0, **kwargs)
            )
        ...
        i = len(chunks)
        rightest_nonzero_chunk_i = (
            i - 1 - max([i for i, e in enumerate(chunks) if e != 0])
        )
        for x in chunks:
            chunk_words = []
            i -= 1
            ...
            if cardinal:
                chunk_words.extend(self.__chunk_cardianl(n3, n2, n1, i, **kwargs))
                if i > 0:
                    chunk_words.append(
            ...
            # ordinal, not joined like 'двухтысячный'
                ...
                chunk_words.extend(self.__chunk_ordinal(n3, n2, n1, i, **kwargs))
                if i > 0:
            ...
            # ordinal, joined
                ...
                chunk_words.extend(self.__chunk_ordinal_join(n3, n2, n1, i, **kwargs))
                if i > 0:
                    chunk_words.append(get_num_element(THOUSANDS_ORD, i, **kwargs))
