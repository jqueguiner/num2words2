...
class Num2Word_VI(object):
    ...
    def _convert_nnn(self, val):
        ...
        if mod > 0 and mod < 10:
            if mod == 5:
                ...
                word = (
                    word != ""
                    and word + "lẻ " + self._convert_nn(mod)
                    or word + self._convert_nn(mod)
                )
        if mod >= 10:
    ...
    def vietnam_number(self, val):
        ...
        for didx, dval in ((v - 1, 1000**v) for v in range(len(denom))):
            if dval > val:
                ...
                ret = self._convert_nnn(lval) + " " + denom[didx]
                if 99 >= r > 0:
                    ret = self._convert_nnn(lval) + " " + denom[didx] + " lẻ"
                if r > 0:
    ...
    def to_currency(
        self, val, currency="VND", cents=True, separator=",", adjective=False
    ):
        """
