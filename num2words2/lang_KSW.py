...
class Num2Word_KSW(Num2Word_Base):
    """S'gaw Karen number-to-words converter."""

    CURRENCY_FORMS = {
        "MMK": (("kyat", "kyat"), ("pya", "pya")),
    ...
    def setup(self):
        ...
        self.exclude_title = ["minus", "decimal", "di"]
        self.ones = [
            "", "ta", "khi", "thuh", "lwi", "yeh",
            "hu", "nwi", "ho", "khwi",
        ]
        self.tens = [
            "", "tasi", "khisi", "thuhsi", "lwisi", "yehsi",
            "husi", "nwisi", "hosi", "khwisi",
        ]
        self.hundred = "yah"
        self.thousand = "klah"
        self.million = "kade"
    ...
    def _int_to_word(self, number):
        ...
        if number < 100:
            ...
            return self.tens[t] + (" di " + self.ones[o] if o else "")
        if number < 1000:
            h, r = divmod(number, 100)
            base = self.ones[h] + " " + self.hundred
            return base + (" di " + self._int_to_word(r) if r else "")
        if number < 1000000:
    ...
    def to_ordinal(self, number):
        return self.to_cardinal(number) + "-tu"

    def to_ordinal_num(self, number):
        return str(number) + "-tu"
