...
class Num2Word_KOK(Num2Word_Base):
    """Konkani number-to-words converter."""
    ...
    def setup(self):
        ...
        self.exclude_title = ["ani", "punto", "rin"]
        self.ones = [
            "", "ek", "don", "tin", "char", "panch",
            "so", "sat", "aath", "nov",
        ]
        self.tens = [
            "", "dha", "vis", "tis", "chalis", "ponnas",
            "saath", "sattar", "aaishi", "novod",
        ]
        self.hundred = "xambhar"
        self.thousand = "hozar"
        self.million = "dosh lakh"
    ...
    def _int_to_word(self, number):
        ...
        if number < 100:
            ...
            return self.tens[t] + (" ani " + self.ones[o] if o else "")
        if number < 1000:
            h, r = divmod(number, 100)
            base = self.ones[h] + " " + self.hundred
            return base + (" ani " + self._int_to_word(r) if r else "")
        if number < 1000000:
    ...
    def to_ordinal(self, number):
        return self.to_cardinal(number) + "vo"
