...
class Num2Word_BO(Num2Word_Base):
    ...
    def _int_to_word(self, number):
        ...
        if number < 10:
            ...
            return (
                self._int_to_word(number // 10000)
                + "ཁྲི་"
                + (self._int_to_word(number % 10000) if number % 10000 else "")
            )
        elif number < 1000000:
            return (
            ...
            return (
                self._int_to_word(number // 1000000)
                + "ས་ཡ་"
                + (self._int_to_word(number % 1000000) if number % 1000000 else "")
            )
        elif number < 100000000:
            return (
                self._int_to_word(number // 10000000)
                + "བྱེ་བ་"
                + (self._int_to_word(number % 10000000) if number % 10000000 else "")
            )
        else:
            return (
                self._int_to_word(number // 100000000)
                + "དུང་ཕྱུར་"
                + (self._int_to_word(number % 100000000) if number % 100000000 else "")
            )
    ...
    def to_currency(
        self, val, currency="CNY", cents=True, separator=" དང་ ", adjective=False
    ):
        """Convert to currency."""
        try:
