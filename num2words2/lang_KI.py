...
class Num2Word_KI(Num2Word_Base):
    """Kikuyu (Gikuyu) number-to-words converter."""

    CURRENCY_FORMS = {
        "KES": (("shilingi", "shilingi"), ("senti", "senti")),
        "USD": (("dora", "dora"), ("senti", "senti")),
    ...
    def setup(self):
        self.negword = "njuru "
        self.pointword = "ona"
        self.exclude_title = ["na", "ona", "njuru"]
        self.ones = [
            "", "ĩmwe", "igĩrĩ", "ithatũ", "inya", "ithano",
            "ithathatũ", "mũgwanja", "inyanya", "kenda",
        ]
        self.tens = [
            "", "ikũmi", "mĩrongo ĩĩrĩ", "mĩrongo ĩtatũ", "mĩrongo ĩna", "mĩrongo ĩtano",
            "mĩrongo ĩtandatũ", "mĩrongo mũgwanja", "mĩrongo ĩnyanya", "mĩrongo kenda",
        ]
        self.hundred = "igana"
        self.thousand = "ngiri"
        self.million = "milioni"
    ...
    def to_currency(self, val, currency="KES", cents=True, separator=" ", adjective=False):
        is_negative = val < 0
        val = abs(val)
        parts = str(val).split(".")
