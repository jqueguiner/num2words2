...
class Num2WordsHYTest(TestCase):
    ...
    def test_currency_special_cases(self):
        ...
        # No cents option
        self.assertEqual(
            num2words(10, lang="hy", to="currency", currency="EUR", cents=False),
            "տասը եվրո",
        )

        # Negative values without fractions
        self.assertEqual(
            num2words(-100, lang="hy", to="currency", currency="EUR", cents=False),
            "մինուս հարյուր եվրո",
        )
    ...
    def test_year_conversion(self):
        ...
        self.assertTrue(
            result_1000.startswith("հազար") and not result_1000.startswith("մեկ հազար")
        )
    ...
    def test_pluralization(self):
        ...
        self.assertEqual(converter.pluralize(0, ("խնձոր", "խնձորներ")), "խնձորներ")
        self.assertEqual(converter.pluralize(1, ("խնձոր", "խնձորներ")), "խնձոր")
        self.assertEqual(converter.pluralize(2, ("խնձոր", "խնձորներ")), "խնձորներ")
        self.assertEqual(converter.pluralize(11, ("խնձոր", "խնձորներ")), "խնձորներ")
        self.assertEqual(converter.pluralize(21, ("խնձոր", "խնձորներ")), "խնձոր")
        self.assertEqual(converter.pluralize(22, ("խնձոր", "խնձորներ")), "խնձորներ")
        self.assertEqual(converter.pluralize(31, ("խնձոր", "խնձորներ")), "խնձոր")

        # Different plural forms
        self.assertEqual(converter.pluralize(0, ("տարի", "տարիներ")), "տարիներ")
        self.assertEqual(converter.pluralize(1, ("տարի", "տարիներ")), "տարի")
        self.assertEqual(converter.pluralize(5, ("տարի", "տարիներ")), "տարիներ")
    ...
    def test_merge_method(self):
        ...
        for snum, nnum in [(500, 5), (800, 8), (900, 9)]:
            val = converter.merge((f"base_{snum}", snum), (f"num_{nnum}", nnum))
        ...
        result_replace = test_str_with_pattern.replace("հազար միլիոն", "միլիարդ")
        ...
        for hundred in [100, 200, 300, 400, 500, 600, 700, 800, 900]:
            for digit in range(1, 10):
                result = converter.merge(
                    (f"test_{hundred}", hundred), (f"test_{digit}", digit)
                )
    ...
    def test_cardinal_million_grouping(self):
        ...
        expected = (
            "տասը միլիոն չորս հարյուր հիսունհինգ հազար " "երեք հարյուր իննսուն յոթ"
        )
        self.assertEqual(result, expected)
