...
class TestNumWord(TestCase):
    ...
    def test_10000_to_99999(self):
        ...
        self.assertEqual(num2words(84573, lang="th"), "แปดหมื่นสี่พันห้าร้อยเจ็ดสิบสาม")

    def test_100000_to_999999(self):
        self.assertEqual(
            num2words(153247, lang="th"), "หนึ่งแสนห้าหมื่นสามพันสองร้อยสี่สิบเจ็ด"
        )
        self.assertEqual(
            num2words(562442, lang="th"), "ห้าแสนหกหมื่นสองพันสี่ร้อยสี่สิบสอง"
        )
        self.assertEqual(
            num2words(999999, lang="th"), "เก้าแสนเก้าหมื่นเก้าพันเก้าร้อยเก้าสิบเก้า"
        )

    def test_more_than_million(self):
        self.assertEqual(num2words(1000000, lang="th"), "หนึ่งล้าน")
        self.assertEqual(num2words(1000001, lang="th"), "หนึ่งล้านเอ็ด")
        self.assertEqual(
        ...
        self.assertEqual(
            num2words(1000000000000000001, lang="th"), "หนึ่งล้านล้านล้านเอ็ด"
        )

    def test_decimal(self):
        self.assertEqual(num2words(0.0, lang="th"), "ศูนย์")
        self.assertEqual(num2words(0.0038, lang="th"), "ศูนย์จุดศูนย์ศูนย์สามแปด")
        self.assertEqual(num2words(0.01, lang="th"), "ศูนย์จุดศูนย์หนึ่ง")
        self.assertEqual(num2words(1.123, lang="th"), "หนึ่งจุดหนึ่งสองสาม")
        self.assertEqual(num2words(35.37, lang="th"), "สามสิบห้าจุดสามเจ็ด")
        self.assertEqual(num2words(1000000.01, lang="th"), "หนึ่งล้านจุดศูนย์หนึ่ง")

    def test_currency(self):
        self.assertEqual(
            num2words(100, lang="th", to="currency", currency="THB"), "ร้อยบาท"
        )
        self.assertEqual(
            num2words(100, lang="th", to="currency", currency="USD"), "ร้อยดอลลาร์"
        )
        self.assertEqual(
            num2words(100, lang="th", to="currency", currency="EUR"), "ร้อยยูโร"
        )

    def test_currency_decimal(self):
        self.assertEqual(num2words(0, lang="th", to="currency"), "ศูนย์บาท")
        self.assertEqual(
            num2words(0.05, lang="th", to="currency"), "ศูนย์บาท ห้าสตางค์"
        )
        self.assertEqual(num2words(0.50, lang="th", to="currency"), "ห้าสิบสตางค์")
        self.assertEqual(num2words(0.99, lang="th", to="currency"), "เก้าสิบเก้าสตางค์")
        self.assertEqual(num2words(100, lang="th", to="currency"), "ร้อยบาท")
        self.assertEqual(
