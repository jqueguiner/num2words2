...
class Num2WordsDETest(TestCase):
    ...
    def test_ordinal_at_crucial_number(self):
        ...
        self.assertEqual(num2words(1000000, ordinal=True, lang="de"), "millionste")
        self.assertEqual(num2words(2000000, ordinal=True, lang="de"), "zweimillionste")
        self.assertEqual(num2words(1000000000, ordinal=True, lang="de"), "milliardste")
        self.assertEqual(
    ...
    def test_cardinal_for_decimal_number(self):
        self.assertEqual(num2words(3.486, lang="de"), "drei Komma vier acht sechs")
    ...
    def test_currency_eur(self):
        for test in TEST_CASES_TO_CURRENCY_EUR:
            self.assertEqual(
                num2words(test[0], lang="de", to="currency", currency="EUR"), test[1]
            )

    def test_currency_usd(self):
        for test in TEST_CASES_TO_CURRENCY_USD:
            self.assertEqual(
                num2words(test[0], lang="de", to="currency", currency="USD"), test[1]
            )

    def test_currency_dem(self):
        for test in TEST_CASES_TO_CURRENCY_DEM:
            self.assertEqual(
                num2words(test[0], lang="de", to="currency", currency="DEM"), test[1]
            )

    def test_currency_gbp(self):
        for test in TEST_CASES_TO_CURRENCY_GBP:
            self.assertEqual(
                num2words(test[0], lang="de", to="currency", currency="GBP"), test[1]
            )
    ...
    def test_year_before_2000(self):
        self.assertEqual(
            num2words(1780, to="year", lang="de"), "siebzehnhundertachtzig"
        )
