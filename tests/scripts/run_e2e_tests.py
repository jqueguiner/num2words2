...
def run_single_test(test_case: dict) -> Tuple[str, str, str]:
    ...
    try:
        ...
        conversion_type = test_case.get(
            "to", test_case.get("conversion_type", "cardinal")
        ).strip()
...
def run_tests(csv_file: str, limit: int = None, verbose: bool = False) -> TestResult:
    ...
    # If file doesn't exist, try looking in tests/data directory
    if not csv_path.exists():
        # Check if we're in the project root
        tests_csv = Path("tests") / "data" / csv_file
        if tests_csv.exists():
            csv_path = tests_csv
        # Check if the file exists relative to the script
        elif (
            Path(csv_file).name == csv_file
            and (Path(__file__).parent.parent / "data" / csv_file).exists()
        ):
        ...
        for i, test_case in enumerate(reader, 1):
            ...
            if status == "PASS":
                ...
                results.failures.append(
                    {"test": test_case, "actual": actual, "error": error}
                )
            else:  # ERROR
                results.errors += 1
                ...
                results.errors_list.append({"test": test_case, "error": error})
            ...
            if verbose and status != "PASS":
                ...
                print(
                    f"  Expected: {test_case.get('expected', test_case.get('expected_output', ''))}"
                )
                if actual:
...
def print_summary(results: TestResult, detailed: bool = False):
    ...
    # Success rate
    success_rate = (
        results.passed / (total - results.skipped) * 100
    ...
    if detailed:
        ...
        for conv_type in sorted(results.by_type.keys()):
            ...
            print(
                f"{conv_type:12} - Total: {total_type:6,} | "
                f"PASS: {stats['passed']:6,} ({success:.1f}%) | "
                f"FAIL: {stats['failed']:6,} | "
                f"ERR: {stats['errors']:6,}"
            )
        ...
        if results.failures:
            ...
            for failure in results.failures[:10]:
                ...
                print(
                    f"  Expected: {test.get('expected', test.get('expected_output', ''))}"
                )
...
def main():
    ...
    parser.add_argument("--limit", "-l", type=int, help="Limit number of tests to run")
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed output for failures"
    )
    parser.add_argument(
        "--detailed", "-d", action="store_true", help="Show detailed summary statistics"
    )
    parser.add_argument(
        "--quick", "-q", action="store_true", help="Quick test with first 100 cases"
    )
